...
async function run() {
    try {
        ...
        if (!userID || !apiKey) {
            throw new Error('You need to pass the userID and the apiKey as input to the function to deploy your files.');
        }
        const basedJsonPath = join(process.cwd(), 'based.json');
        if (!existsSync(basedJsonPath)) {
            throw new Error('Was not possible to find the "based.json" file in the branch. Add the file and try again.');
        }
        const basedJson = JSON.parse(readFileSync(basedJsonPath, 'utf-8'));
        const { org, project, env } = basedJson;
        // #branch check for env
        if (!org || !project || !env) {
            throw new Error('Was not possible to read the "based.json" file in the branch.');
        }
